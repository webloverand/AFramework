...
namespace AFramework
{
    public class Timer
    {
        ...
        {
            if (null == driver) driver = TimerDriver.Instance; //初始化Time驱动
            _time = time;
            _loop = loop;
            ...
            {
                if (showLog) Debug.LogErrorFormat("【TimerTrigger（容错）】:存在相同的标识符【{0}】！", flag);
            }
    ...
    [MonoSingletonPath("[AFramework]/TimerManager")]
    public class TimerDriver : MonoSingletonWithNewObject<TimerDriver>
    {
        private void Update()
        {
